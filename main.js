...
async function loadSystems() {
    ...
    if (!systemsGrid) {
        console.error('systemsGrid element not found');
        return;
    }

    // Show loading state
    systemsGrid.innerHTML = '<div class="loading">جاري تحميل المنتجات...</div>';

    // Try to load products from Firebase first
    let productsToShow = [];
    try {
        if (typeof FirebaseUtils !== 'undefined' && FirebaseUtils.getDocuments) {
            ...
            if (result.success && result.data.length > 0) {
                ...
                console.log('تم تحميل المنتجات من Firebase:', productsToShow.length);
            }
        }
    } catch (error) {
    ...
    if (productsToShow.length === 0) {
        ...
        console.log('استخدام البيانات الثابتة:', productsToShow.length);
    }
