...
// Set Arabic locale for Firestore with proper configuration
try {
    db.settings({
        timestampsInSnapshots: true,
        experimentalForceLongPolling: false, // تعطيل Long Polling لتحسين الأداء
        merge: true // إضافة merge لتجنب تحذير الإعدادات
    });
} catch (error) {
    console.warn('تحذير: لا يمكن تطبيق إعدادات Firestore:', error);
}
...
// Default UI update functions if not defined elsewhere
function updateUIForAuthenticatedUser(user) {
    ...
    if (accountBtn) {
        ...
        accountBtn.textContent = user.displayName || user.email || 'حسابي';
    }
    
    // Update user-specific content
    const userNameElements = document.querySelectorAll('.user-name');
    userNameElements.forEach(element => {
        element.textContent = user.displayName || user.email || 'المستخدم';
    });
}
